#include "manaFatigue.h"

ManaFatigue::ManaFatigue(std::string name, Board* board) : Enchantment{name, board} {
	enchantmentName = "Mana Fatigue";
	enchantmentDescription = "Enchanted minion's activated ability costs 2 more";
	cost = 0;
}

void ManaFatigue::play(int owner, int targetPlayer, int minion, bool actOnRitual){
	...
	auto m = std::make_shared<ManaFatigue>(name,board);
...
void ManaFatigue::useAbility(int activePlayer, int targetPlayer, int minion) { this->minion->useAbility(activePlayer,targetPlayer,minion); }
void ManaFatigue::useTriggered(int owner, int playedMinion, bool isOwnerActive, When when) { minion->useTriggered(owner,playedMinion,isOwnerActive,when); }

std::shared_ptr<AbstractMinion> ManaFatigue::getMinion(bool forDisplay) {
